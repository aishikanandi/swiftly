  ...
  io.on("connection", (socket) => {
    ...
    socket.on("nearestdata", async (data) => {
      ...
      try { 
        ...
        const driverdata = await driverModel.find({ city: cityId, servicetype: serviceId, assign: "0" });
    ...
    socket.on('ridecompleted', async (data) => {
      ...
      try {
        ...
        driverModel.findByIdAndUpdate(driverId, {$set: {assign: "0"}})
        ...
        let toPhoneNumber = `+91 81793 54811`
    ...
    socket.on('driverfree', async (data) => {
      ...
      try {
        ...
        const ride = await createrideModel.find(rideId)
    ...
    //               },
    //             ]);
    //             console.log("driver being assigned")
    ...
    async function myTask() {
      try {
        console.log("Fetching rides to be assigned...");
        const asigningrides = await createrideModel.find({ ridestatus: 1, nearest: false });
        const nearestridedata = await createrideModel.find({
          $and: [{ $or: [{ ridestatus: 1 }, { ridestatus: 2 }, { ridestatus: 8 }] }, { nearest: true }],
        });
        console.log(`Found ${asigningrides.length} rides needing assignment and ${nearestridedata.length} nearest rides.`);
    
        //------ For Single Assign Driver -------//
        if (asigningrides.length > 0) {
          console.log("Processing rides that need a single driver assignment...");
          for (const data of asigningrides) {
            if (data.assigningTime) {
              let currenttime = Date.now();
              let assignedTime = data.assigningTime;
              resulttimeout = Math.floor((currenttime - assignedTime) / 1000);
              console.log(`Ride ID: ${data._id} | Timeout: ${resulttimeout >= RideTimeOut}`);
    
              if (resulttimeout >= RideTimeOut) {
                console.log(`Timeout for ride ID: ${data._id}. Unassigning driver ID: ${data.driverId}...`);
    
                const drivernewdata = await driverModel.findByIdAndUpdate(data.driverId, { $set: { assign: "0" } }, { new: true });
                const ridenewdata = await createrideModel.findByIdAndUpdate(data._id, {
                  $unset: { driverId: "" },
                  $set: { ridestatus: 0 },
                }, { new: true });
    
                console.log("Driver unassigned and ride status reset. Emitting events...");
                io.emit("runningrequestreject", drivernewdata, ridenewdata);
                io.emit("timeoutdata", { success: true, message: "timeoutdata", ridenewdata, drivernewdata });
              } else {
                console.log(`Ride ID: ${data._id} | No timeout yet, skipping...`);
              }
            }
          }
        } else {
          console.log("No rides requiring single driver assignment found.");
        }
    
        //------ For Nearest Driver Assignments -------//
        if (nearestridedata.length > 0) {
          console.log("Processing nearest ride assignments...");
          for (const data of nearestridedata) {
            let currenttime = Date.now();
            let assignedTime = data.assigningTime;
            resulttimeout = Math.floor((currenttime - assignedTime) / 1000);
            console.log(`Nearest Ride ID: ${data._id} | Timeout: ${resulttimeout >= RideTimeOut}`);
    
            if (resulttimeout >= RideTimeOut) {
              console.log(`Timeout for nearest ride ID: ${data._id}. Finding new driver...`);
              const city_id = new mongoose.Types.ObjectId(data.cityId);
              const vehicle_id = new mongoose.Types.ObjectId(data.serviceId);
              const nearestdriversArray = [...new Set(data.nearestArray)];
              console.log(`Looking for drivers not in nearestArray: ${nearestdriversArray}`);
    
              const olddriver = await driverModel.findByIdAndUpdate(data.driverId, { $set: { assign: "0" } }, { new: true });
              console.log("Old driver unassigned:", olddriver);
              ...
              const pendingdrivertoassign = await nonassigndriverdata.exec();
              console.log("Drivers available for assignment:", pendingdrivertoassign);
    
              if (pendingdrivertoassign.length > 0) {
                const randomIndex = Math.floor(Math.random() * pendingdrivertoassign.length);
                const randomdriver = pendingdrivertoassign[randomIndex];
                console.log(`Assigning new driver ID: ${randomdriver._id} to ride ID: ${data._id}`);
                ...
                console.log("New driver assigned and ride updated. Emitting events...");
                notificationCounter++;
                io.emit("pushnotification", { notificationCounter });
                io.emit("whendriverisnearest", { success: true, olddriver, newdriver, finalresult });
              } else {
                console.log("No available drivers, checking assigned drivers...");
                // Further logic for checking assigned drivers
              }
            } else {
              console.log(`Nearest ride ID: ${data._id} | No timeout yet, skipping...`);
            }
          }
        } else {
          console.log("No nearest rides found to process.");
        }
      } catch (error) {
        console.error("Error in myTask:", error);
      }
