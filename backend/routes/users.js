...
const stripe = require('stripe')('sk_test_51NZeiUANXK9scyulpxLuZ2UL5HvCqJBALzHeOfXQxDljxeroEWHfM9Gz9ZhdOau5mV9tyHQx36q5g6HcVPAvlXiA00iaZTcfFv')
  ...
  userRoutes.post('/adduser', async (req, res, next) => {
    ...
    const { username, useremail, countrycode, userphone, userType } = req.body;
    let newUser;
    try {
      if (!req.file) {
        newUser = new userModel({ username: username, useremail: useremail, countrycode: countrycode, userphone: userphone, userType: userType });
      }else{
        const profile = req.file.filename;
        // console.log(profile);
        newUser = new userModel({ profile: profile, username: username, useremail: useremail, countrycode: countrycode, userphone: userphone, userType: userType });
      }
      ...
      if(error.keyPattern){
        console.log(error)
        return res.status(500).json({success: false, message: "User Already Registered", error: error.message});
      }
...
userRoutes.post('/userdata/email', async (req, res) => {
  try {
