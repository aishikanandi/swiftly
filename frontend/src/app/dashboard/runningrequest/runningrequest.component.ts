...
export class RunningrequestComponent {
  ...
  driverId: any;
  rideId: any;
  driverData= JSON.parse(localStorage.getItem('userDetails')||'');
  map: any;
  ...
  ngOnInit() {
    ...
    // this.listenUserRejectRunningRequest()
  }
  ...
  getRunningData() {
    ...
    this._socketservice.listenGetRunning().subscribe((data: any) => {
      this.assignedArray = data.alldata.filter((item: any) => 
        item.driverDetails.driverphone === this.driverData.userphone
      );
