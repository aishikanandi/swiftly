...
export class CreaterideComponent {
  ...
  ngOnInit() {
    ...
    this.initMap();
    this.getUserDetails();
    this.numbersArray = Array.from({ length: 20 }, (v, k) => k + 1); 
  }
  ...
  getUserDetails() {
    this.isNext = true;
        ...
        this._city.getcity(this.page, this.citylimit).subscribe({
          next: (resp: any) => {
  ...
  onSelectServiceType(serviceType: any) {
    ...
    if (this.selectedVehicle) {
      ...
      // Calculate the fare for the selected vehicle
      this.calculateFare(this.selectedVehicle, this.cityIndex);
  
      // Trigger change detection to update the fare in the UI
      this.cdr.detectChanges();
    }
  }
  
  
  onSelectServiceNum(num: string) {
    const selectedNum = Number(num);
  
    if (isNaN(selectedNum) || selectedNum <= 0) {
      return; // If the input is invalid, exit early
    }
  
    // Multiply the original fare (stored in fareForOneVehicle) by the selected number
    this.vehiclesPricing.forEach((vehiclePricing: any) => {
      if (!vehiclePricing.fareForOneVehicle) {
        // If fareForOneVehicle is not stored, we should store it first (optional safeguard)
        vehiclePricing.fareForOneVehicle = vehiclePricing.estimateFare;
      }
  
      // Now multiply the fare by selectedNum
      vehiclePricing.estimateFare = vehiclePricing.fareForOneVehicle * selectedNum;
    });
  ...
  handleRadioChange() {
    ...
    if (this.selectedOption === "bookNow") {
      ...
      if(!this.isDateTimeValid&&!this.invalidToaster){
        this.invalidToaster =!this.invalidToaster;
